import { CardCollection, Page } from '../src/core/card-collection.js';
import { MCard } from '../src/core/mcard.js';
import { HashAlgorithm } from '../src/config/config_constants.js';
import { Buffer } from 'buffer';

// Mock the HashValidator
jest.mock('../src/core/hash/validator.js', () => {
  // Keep track of hashes to ensure uniqueness
  const hashes = new Map();
  
  return jest.fn().mockImplementation(() => ({
    validate: jest.fn().mockResolvedValue(true),
    computeHash: jest.fn().mockImplementation((content, algorithm) => {
      // Create a unique hash based on content
      const contentStr = content.toString('utf-8');
      if (!hashes.has(contentStr)) {
        ...
        hashes.set(contentStr, `mocked-hash-${Math.abs(hash).toString(16)}`);
      }
      return hashes.get(contentStr);
    })
  }));
});
  ...
  search_by_string(search_string, page_number = 1, page_size = 10) {
    ...
    const matchingCards = allCards.filter(card => {
      ...
      if (card.content instanceof Uint8Array) {
        ...
        contentStr = new TextDecoder('utf-8').decode(card.content);
      } else if (typeof card.content === 'string') {
        contentStr = card.content;
      } else if (card.content && typeof card.content.toString === 'function') {
        contentStr = card.content.toString('utf-8');
      } else {
        contentStr = String(card.content);
      }
...
describe('CardCollection', () => {
  ...
  describe('Pagination Methods', () => {
    ...
    test('search_by_string should return matching cards', () => {
      // Add a card with specific content to search for
      const testCard = new MCard('Specific Content 1');
      cardCollection.add(testCard);
      
      // Search for the specific content
      const page = cardCollection.search_by_string('Specific Content 1');
      
      expect(page.items.length).toBeGreaterThan(0);
      page.items.forEach(card => {
        // Convert Uint8Array to string for comparison
        const content = card.content;
        const contentStr = content instanceof Uint8Array 
          ? new TextDecoder('utf-8').decode(content)
          : String(content);
        
        expect(contentStr).toContain('Specific Content 1');
      });
    });
